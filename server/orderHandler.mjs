...
export function verifyAdminToken(token) {
  ...
  try {
    ...
    return String(data.u || "").toLowerCase() === adminUser().toLowerCase() && Number(data.exp) > Date.now();
  } catch {
    return false;
  }
}
...
export function adminLogin(username, password) {
  ...
  if (String(username || "").toLowerCase() !== user.toLowerCase() || password !== pass) {
...
export function createOrder(payload) {
  ...
  // Defense in depth: the UI already hides "افزودن به سبد" for out-of-stock items.
  // Only an explicitly unavailable line is refused; unknown availability stays allowed.
  const unavailable = normalizedItems.filter((item) => {
    const value = String(item.availability || "").trim().toLowerCase();
    return value === "ناموجود" || value === "out of stock" || value === "out_of_stock" || value === "unavailable";
  });
