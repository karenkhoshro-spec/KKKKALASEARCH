...
const otherRules: [string, string, string[]][] = [
  ...
  ["bucket", "سطل", ["سطل", "درب سطل"]], ["basin-bathtub", "لگن و وان", ["لگن", "وان"]], ["flower-pot", "گلدان", ["گلدان", "زیر گلدان"]],
  ["shopping-basket-other", "زنبیل", ["زنبیل"]], ["oval-basket", "سبد بیضی", ["سبد بیضی"]], ["janani", "جانانی", ["جانانی"]], ["organizer", "سبد و نظم‌دهنده", ["نظم دهنده", "نظم‌دهنده", "باکس", "جعبه نظم", "جعبه همه کاره", "فایل"]],
...
const colorTokens: Record<string, string> = {
  "صورتی": "#f9a8d4", "طوسی": "#94a3b8", "خاکستری": "#6b7280", "کرم": "#d6c7a1", "موکا": "#8b6f5a", "وانیلی": "#f3e5ab", "سفید": "#f8fafc", "قرمز": "#ef4444", "سبز": "#22c55e", "آبی": "#3b82f6", "مشکی": "#111827", "سیاه": "#111827", "زرد": "#eab308", "نارنجی": "#f97316", "بنفش": "#a855f7", "شفاف": "#e2e8f0", "بژ": "#d6c7a1", "طلایی": "#d4a72c", "مسی": "#b87333"
};
function extractColor(value: string) { const token = Object.keys(colorTokens).find((name) => value.includes(name)); return token ? colorTokens[token] : undefined; }
function toVariation(row: ProductSourceRow): ProductVariation { return { id: row.variant_sku, name: localized(row.variant_name || row.product_name), color: extractColor(row.variant_name || ""), sku: row.variant_sku || undefined, price: numberOrUndefined(row.price), image: row.main_image || row.image_1 || row.image_2 || undefined, stockCount: numberOrUndefined(row.stock), packQuantity: numberOrUndefined(row.pack_quantity), inStock: row.availability === "موجود", url: row.ashkan_url || undefined, technicalSpec: row.technical_spec || undefined }; }
