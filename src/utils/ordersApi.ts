...
export function buildOrderItems(items: CartItem[]): OrderItemPayload[] {
  return items.map((item) => {
    ...
    // Availability is snapshotted from the real CSV inventory at order time so the
    // admin sees what the customer saw; the API re-validates it and rejects out-of-stock lines.
    const inStock = variation ? variation.inStock : product?.inStock ?? false;
    return {
      ...
      availability: inStock ? "موجود" : "ناموجود",
      stockCount: variation?.stockCount ?? product?.stockCount ?? 0,
    };
  });
}
